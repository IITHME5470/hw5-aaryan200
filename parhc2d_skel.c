...
void grid(int nx, int nxglob, int istglob, int ienglob, double xstglob, double xenglob, double *x, double *dx)
{
  ...
  //*dx = (xen-xst)/(double)(nx-1);              // this won't work because dx should be computed from the global domain
  *dx = (xenglob - xstglob)/(double)(nxglob-1);  // this will work because xenglob and xstglob are global
...
void enforce_bcs(int nx, int nxglob, int ny, int nyglob, int istglob, int ienglob, int jstglob, int jenglob, double *x, double *y, double **T)
{
  int i, j;
...
void set_initial_condition(int nx, int nxglob, int ny, int nyglob, int istglob, int ienglob, int jstglob, int jenglob, double *x, double *y, double **T, double dx, double dy)
{
  int i, j;
  double del=1.0;
  ...
  enforce_bcs(nx,nxglob,ny,nyglob,istglob,ienglob,jstglob,jenglob,x,y,T); //ensure BCs are satisfied at t = 0
}
...
void get_rhs(int nx, int nxglob, int ny, int nyglob, int istglob, int ienglob, int jstglob, int jenglob, double dx, double dy, double *xleftghost, double *xrightghost, double *ybotghost, double *ytopghost, double kdiff, double *x, double *y, double **T, double **rhs)
{
  ...
  // bot-right corner
  i = nx-1; j = 0;
  if (istglob==nxglob-1 || jstglob==0)  //processors adjacent to the right or bottom ends of the domain
      rhs[i][j] = 0.0;
  else
      rhs[i][j] = kdiff*(xrightghost[j]+T[i-1][j]-2.0*T[i][j])/dxsq +   // T[i+1][j] replaced with xrightghost
                  kdiff*(T[i][j+1]+ybotghost[i]-2.0*T[i][j])/dysq;     // T[i][j-1] replaced with ybotghost
  ...
  // top-right corner
  i = nx-1; j = ny-1;
  if (istglob==nxglob-1 || jenglob==nyglob-1)  //processors adjacent to the right or top ends of the domain
      rhs[i][j] = 0.0;
  else
      rhs[i][j] = kdiff*(xrightghost[j]+T[i-1][j]-2.0*T[i][j])/dxsq +   // T[i+1][j] replaced with xrightghost
                  kdiff*(ytopghost[i]+T[i][j-1]-2.0*T[i][j])/dysq;     // T[i][j+1] replaced with ytopghost
...
void halo_exchange_2d_x(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *xleftghost, double *xrightghost, double *sendbuf_x, double *recvbuf_x)
{
  ...
  MPI_Recv(recvbuf_x, ny, MPI_DOUBLE, right_nb, 10, MPI_COMM_WORLD, &status);

  MPI_Send(sendbuf_x, ny, MPI_DOUBLE, left_nb, 10, MPI_COMM_WORLD);

  // unpack recv buffer
  for(j = 0; j < ny; j++) {
    xrightghost[j] = (right_nb != MPI_PROC_NULL) ? recvbuf_x[j] : 0.0;
  }
  ...
  MPI_Recv(recvbuf_x, ny, MPI_DOUBLE, left_nb, 20, MPI_COMM_WORLD, &status);

  MPI_Send(sendbuf_x, ny, MPI_DOUBLE, right_nb, 20, MPI_COMM_WORLD);

  // unpack recv buffer
  for(j = 0; j < ny; j++) {
    xleftghost[j] = (left_nb != MPI_PROC_NULL) ? recvbuf_x[j] : 0.0;
  }
}
...
void halo_exchange_2d_y(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *ybotghost, double *ytopghost, double *sendbuf_y, double *recvbuf_y)
{
  ...
  MPI_Recv(recvbuf_y, nx, MPI_DOUBLE, top_nb, 30, MPI_COMM_WORLD, &status);

  MPI_Send(sendbuf_y, nx, MPI_DOUBLE, bot_nb, 30, MPI_COMM_WORLD);

  // unpack recv buffer
  for(i = 0; i < nx; i++) {
    ytopghost[i] = (top_nb != MPI_PROC_NULL) ? recvbuf_y[i] : 0.0;
  }
  ...
  MPI_Recv(recvbuf_y, nx, MPI_DOUBLE, bot_nb, 40, MPI_COMM_WORLD, &status);

  MPI_Send(sendbuf_y, nx, MPI_DOUBLE, top_nb, 40, MPI_COMM_WORLD);

  // unpack recv buffer
  for(i = 0; i < nx; i++) {
    ybotghost[i] = (bot_nb != MPI_PROC_NULL) ? recvbuf_y[i] : 0.0;
  }
}
...
void timestep_FwdEuler(int rank, int size, int rank_x, int rank_y, int px, int py, int nx, int nxglob, int ny, int nyglob, int istglob, int ienglob, int jstglob, int jenglob, double dt, double dx, double dy, double *xleftghost, double *xrightghost, double *ybotghost, double *ytopghost, double kdiff, double *x, double *y, double **T, double **rhs, double *sendbuf_x, double *recvbuf_x, double *sendbuf_y, double *recvbuf_y)
{
  ...
  // set Dirichlet BCs
  enforce_bcs(nx,nxglob,ny,nyglob,istglob,ienglob,jstglob,jenglob,x,y,T);
...
int main(int argc, char** argv)
{
  ...
  double step_start, step_end, step_time, max_step_time;
  ...
  if(rank==0)
  {
    ...
    it_print = num_time_steps/5;            // write out every t_print time units
  ...
  set_initial_condition(nx,nxglob,ny,nyglob,istglob,ienglob,jstglob,jenglob,x,y,T,dx,dy); // initial condition
  ...
  for(it=0; it<num_time_steps; it++)
  {
    ...
    // Compute the maximum time across all processes for this step
    MPI_Reduce(&step_time, &max_step_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);

    if (rank == 0) total_wall_time += max_step_time;
  ...
  // Print final timing results from rank 0
  if (rank == 0) {
    double avg_time_per_step = total_wall_time / num_time_steps;
    printf("Average time per step: %lf sec\n", avg_time_per_step);
  }
